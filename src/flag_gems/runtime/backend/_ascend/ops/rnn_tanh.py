...
@libentry()
@triton.jit
def rnn_tanh_packed_recurrent_ascend_kernel(
    hidden_ptr,
    weight_hh_ptr,
    linear_ptr,
    output_ptr,
    ...
    """Consume a packed input projection and update one recurrent step."""
    b_offs = tl.program_id(0) * BLOCK_B + tl.arange(0, BLOCK_B)
    h_offs = tl.arange(0, BLOCK_H)
    ...
    previous = tl.load(hidden_ptr + hidden_offsets, mask=mask, other=0.0)
    packed_offsets = (row_offset + b_offs[:, None]) * hidden_size + h_offs[None, :]
    tl.store(previous_ptr + packed_offsets, previous, mask=mask)
    ...
    recurrent = tl.dot(previous, tl.trans(weight)).to(tl.float32)
    input_linear = tl.load(linear_ptr + packed_offsets, mask=mask, other=0.0)
    current = flag_gems_tanh_scalar(input_linear + recurrent).to(weight.dtype)
    current = tl.where(mask, current, 0.0)
    tl.store(hidden_ptr + hidden_offsets, current, mask=mask)
    output_offsets = (
        (row_offset + b_offs[:, None]) * output_feature_size
        + direction * hidden_size
...
def _launch_packed_forward(
    ...
    for layer in range(num_layers):
        ...
        for direction in range(directions):
            ...
            input_linear = None
            compiled_recurrent = None
            if use_ascend_prelinear:
                input_linear = _empty((rows, hidden_size), data, dtype=torch.float32)
                block_m = 16
                block_n = 32 if hidden_size <= 32 else 64
                block_k = 32 if max(current_input_size, hidden_size) <= 32 else 64
                linear_grid = (
                ...
                block_b = min(16, triton.next_power_of_2(max_batch))
                block_h = max(16, triton.next_power_of_2(hidden_size))
                recurrent_grid = (triton.cdiv(max_batch, block_b),)
                recurrent_direct_grid = (*recurrent_grid, 1, 1)
            time_indices = (
            ...
            for time_index in time_indices:
                ...
                if use_ascend_prelinear:
                    if compiled_recurrent is not None:
                        if _COMPILED_RUNNER_TAKES_CONSTEXPRS:
                            compiled_recurrent(
                                hidden,
                                weight_hh,
                        ...
                        compiled_kernel, _ = rnn_tanh_packed_recurrent_ascend_kernel[
                            recurrent_grid
                        ](
                            hidden,
                            weight_hh,
                        ...
                        if row_offset > 1 and active_batch > 1:
                            compiled_recurrent = compiled_kernel[recurrent_direct_grid]
                elif use_dot:
                    block_b = 16
...
def rnn_tanh(
    ...
    """
    logger.debug("GEMS_ASCEND RNN_TANH")
...
def rnn_tanh_data(
    ...
    logger.debug("GEMS_ASCEND RNN_TANH_DATA")
    if data.ndim != 2:
