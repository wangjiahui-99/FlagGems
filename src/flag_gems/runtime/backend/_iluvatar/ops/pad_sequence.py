...
"""Iluvatar implementation of pad_sequence."""
...
def pad_sequence(sequences, batch_first=False, padding_value=0.0):
    logger.debug("GEMS_ILUVATAR PAD_SEQUENCE")
    batch = len(sequences)
    if batch == 0:
    ...
    for sequence in sequences:
        ...
        # The kernel addresses flat buffers. Keep the launch arguments 1D so
        # Corex does not infer matrix SME layouts for this copy-only kernel.
        seqs.append(sequence.view(-1))
    ...
    kernel_output = output.view(-1)
    if dtype == torch.float64:
        kernel_output = kernel_output.view(torch.int64)
